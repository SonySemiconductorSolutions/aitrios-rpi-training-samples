...
# import factory
from .modeling import nn_blocks
from .modeling import nn_layers

layers = tf.keras.layers

USE_SDSP_CONVERTER = True

#  pylint: disable=pointless-string-statement


class Conv2DBNBlock_new():
  ...
  def __call__(self, inputs, training=None):
    ...
    if self._use_normalization:
      x = self._norm0(x)
    return tf_utils.get_activation(
        self._activation, use_keras_layer=True)(x)


class DepthwiseSeparableConvBlock_new():
...
MNV1_BLOCK_SPECS = {
    ...
    'block_specs': [
        ('convbn', 3, 2, 24, False),
        ('depsepconv', 3, 1, 48, False),
        ('depsepconv', 3, 2, 96, False),
        ('depsepconv', 3, 1, 96, True),
        ('depsepconv', 3, 2, 192, False),
        ('depsepconv', 3, 1, 192, True),
        ('depsepconv', 3, 2, 384, False),
        ('depsepconv', 3, 1, 384, False),
        ('depsepconv', 3, 1, 384, False),
        ('depsepconv', 3, 1, 384, False),
        ('depsepconv', 3, 1, 384, False),
        ('depsepconv', 3, 1, 384, True),
        ('depsepconv', 3, 1, 384, False),
        ('depsepconv', 3, 1, 384, True),
    ]
}
...
class MobileNet(tf.keras.Model):
  ...
  def __init__(
      self,
      model_id: str = 'MobileNetV1',
      filter_size_scale: float = 1.0,
      input_specs: tf.keras.layers.InputSpec = layers.InputSpec(
          shape=[None, 353, 481, 3]),
  ...
  def mobilenet_base(self,input_tensor):

    x,_,e=self._mobilenet_base(input_tensor)
  ...
  def _mobilenet_base(self,
    ...
    for i, block_def in enumerate(self._decoded_specs):
      if USE_SDSP_CONVERTER:
        self.modify_for_sdspconv(block_def)
      ...
      if block_def.block_fn == 'convbn':
        ...
        net = Conv2DBNBlock_new(
            input_shape=net.shape,
        ...
        net = DepthwiseSeparableConvBlock_new(
            input_shape=net.shape,
            filters=block_def.filters,
            kernel_size=block_def.kernel_size,
  ...
  def modify_for_sdspconv(self, block_def):
    if block_def.activation == "relu6":
      block_def.activation = "relu"
...
def get_mobilenetv1_base(input_tensor=None, output_stride=None, return_model=None):
  basemodel_obj=MobileNet(output_stride=None)
  x=basemodel_obj.mobilenet_base(input_tensor)
  if return_model:
    return basemodel_obj.mobilenet_base
  else:
    return x
