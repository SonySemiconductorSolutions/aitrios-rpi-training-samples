...
def map_SSS_to_personlab(keypoints):
    permute = config.get_permute()
...
def compute_short_offsets(all_keypoints, discs):
    ...
    def copy_with_border_check(map, center, disc):
        ...
        cropped_disc = disc[
            center[1] - r + from_top : center[1] + r + 1 - from_bottom,
        ...
        map[
            ...
            center[0] - r + from_left : center[0] + r + 1 - from_right,
            :,
        ][cropped_disc, :] = kp_circle[
            from_top : 2 * r + 1 - from_bottom,
        ...
        for j in range(len(all_keypoints)):
            if all_keypoints[j][i, 2] > 0:
                copy_with_border_check(
                    offsets[:, :, 2 * i : 2 * i + 2],
                    (all_keypoints[j][i, 0], all_keypoints[j][i, 1]),
                    discs[j][i],
                )
...
def compute_mid_offsets(all_keypoints, discs):
    ...
    for i, edge in enumerate(
        ...
        for j in range(len(all_keypoints)):
            if (
                all_keypoints[j][edge[0], 2] > 0
                and all_keypoints[j][edge[1], 2] > 0
            ):
                ...
                dists = [
                    [
                        all_keypoints[j][edge[1], 0],
                        all_keypoints[j][edge[1], 1],
                    ]
                ] - idx[m, :]
