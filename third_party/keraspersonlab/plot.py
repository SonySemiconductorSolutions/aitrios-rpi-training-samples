...
def plot_poses(img, skeletons, save_path=None):
    ...
    for i in range(config.NUM_KP):
        rgba = np.array(cmap(1 - i / 17.0 - 1.0 / 34))
        rgba[0:3] *= 255
        c = colors[i] if i < len(colors) else [0, 0, 255]
    ...
    for i in range(config.NUM_EDGES):
        ...
        for j in range(len(skeletons)):
            ...
            cv.fillConvexPoly(cur_canvas, polygon, c)
            canvas = cv.addWeighted(canvas, 0.4, cur_canvas, 0.6, 0)

    if save_path is not None:
        image_rgb = cv.cvtColor(canvas[:, :, :], cv.COLOR_BGR2RGB)
